...
const ProfilePage = ({ user, onLogout, onUpdateUser }) => {
  ...
  return (
    <div className="min-h-screen bg-[#080808] text-white flex justify-center items-start pt-8 px-6 antialiased"
      ...
      <style>{`
        ...
        .interact-btn { display: flex; align-items: center; gap: 8px; color: #444; transition: all 0.2s; cursor: pointer; border: none; background: none; outline: none; }
        .interact-btn svg { width: 18px; height: 18px; stroke-width: 2.5; }
      ...
      <div className="w-full max-w-[1100px] flex flex-row gap-6 justify-center">
        <aside className="w-[240px] flex flex-col gap-5 glass-card p-5 rounded-[28px] h-fit sticky top-8 shadow-2xl">
          <div className="flex justify-center py-4 cursor-pointer" onClick={() => setView('feed')}>
            <span className="sss-logo text-4xl font-black italic tracking-tighter select-none">SSS</span>
          </div>
          <nav className="flex flex-col gap-1 mt-2">
            <div onClick={() => setView('feed')} className={`flex items-center gap-3.5 p-3 cursor-pointer rounded-xl transition-all duration-300 ${view === 'feed' ? 'bg-white/5 text-white border-r-2 border-[#ff2a5f]' : 'text-gray-500 hover:text-white hover:bg-white/5'}`}>
              ...
              <span className="text-[14px] font-bold tracking-tight">Лента</span>
            </div>
            <div onClick={() => { setCurrentProfile(user.username); setView('profile'); setActiveTab('posts'); }} className={`flex items-center gap-3.5 p-3 cursor-pointer rounded-xl transition-all duration-300 ${view === 'profile' && isOwnProfile ? 'bg-white/5 text-white border-r-2 border-[#ff2a5f]' : 'text-gray-500 hover:text-white hover:bg-white/5'}`}>
          ...
          <div onClick={onLogout} className="mt-16 border-t border-white/5 pt-5 text-gray-600 hover:text-red-500 cursor-pointer transition-all flex items-center gap-3 px-2 group">
        ...
        <main className="flex-1 max-w-[680px] flex flex-col gap-5">
          {view === 'profile' && (
            <div className="glass-card rounded-[36px] overflow-hidden shadow-2xl border-white/10">
              <div className="h-40 bg-gradient-to-br from-[#111] via-[#1a1a1a] to-[#080808]"></div>
              <div className="px-8 pb-8 relative">
                <div className="flex justify-between items-end -mt-12 mb-6">
                  <div className="relative group">
                    <div className="w-28 h-28 rounded-[28px] bg-[#111] border-[5px] border-[#080808] flex items-center justify-center text-5xl shadow-2xl">
                    ...
                    {isOwnProfile && (
                      <div className="absolute -bottom-1 -right-1 z-50">
                        <button 
                          ...
                          className="w-9 h-9 bg-[#ff2a5f] rounded-xl flex items-center justify-center border-[3px] border-[#080808] hover:scale-110 transition-transform cursor-pointer shadow-lg active:scale-95"
                        >
                        ...
                        {isEmojiPickerOpen && (
                          <div 
                            className="absolute top-full left-0 mt-3 p-2 bg-[#121212] border border-white/10 rounded-2xl shadow-2xl grid grid-cols-5 gap-1 w-48 backdrop-blur-xl z-[100]"
                          ...
                          >
                            {EMOJI_LIST.map((emoji) => (
                              <button
                                ...
                                className="text-xl p-2 hover:bg-white/10 rounded-lg transition-all cursor-pointer hover:scale-125 border-none bg-transparent"
                              >
                                {emoji}
                              </button>
                            ))}
                  ...
                  <div className="flex gap-2 pb-1">
                    {isOwnProfile ? (
                      <button onClick={handleSaveProfile} className={`px-5 py-2 rounded-xl text-[10px] font-black uppercase transition-all border ${isEditing ? 'bg-[#ff2a5f] border-[#ff2a5f] text-white' : 'bg-white/5 border-white/10 text-white hover:bg-white hover:text-black'}`}>
                        {isEditing ? 'Сохранить' : 'Редактировать'}
                      </button>
                    ) : (
                      <button 
                        onClick={handleFollow} 
                        className={`px-6 py-2 rounded-xl text-[10px] font-black uppercase transition-all cursor-pointer border ${profileData.isSubscribed ? 'bg-white/5 border-white/10 text-white' : 'bg-[#ff2a5f] border-[#ff2a5f] text-white shadow-lg shadow-[#ff2a5f]/20 hover:scale-105 active:scale-95'}`}>
                        {profileData.isSubscribed ? 'Отписаться' : 'Подписаться'}
                      </button>
                    )}
                    <button className="bg-gradient-to-r from-[#ff2a5f] to-[#7e22ce] text-white px-5 py-2 rounded-xl text-[10px] font-black uppercase shadow-lg shadow-[#ff2a5f]/20">Premium</button>
                  </div>
                </div>
                    ...
                    {isEditing ? (
                      ...
                      <div>
                        <div className="flex items-center gap-2">
                          <h1 className={`text-2xl font-black tracking-tighter ${hasPremium ? 'premium-nick' : ''}`}>{profileData.username}</h1>
                          <div className="w-4 h-4 bg-[#ff2a5f] rounded-full flex items-center justify-center text-[8px] text-white font-bold shadow-[0_0_10px_rgba(255,42,95,0.4)]">✓</div>
                        </div>
                        <p className="text-gray-500 font-bold text-sm">@{profileData.handle}</p>
                      </div>
                    )}
                    
                    <div className="flex gap-6 mt-4">
                      <p className="text-sm"><span className="font-black text-white">{profileData.followers}</span> <span className="text-gray-500 font-bold uppercase text-[10px] tracking-wider ml-1">Подписчики</span></p>
                      <p className="text-sm"><span className="font-black text-white">{profileData.following}</span> <span className="text-gray-500 font-bold uppercase text-[10px] tracking-wider ml-1">Подписки</span></p>
                    </div>
                    <div className="text-[9px] text-gray-600 font-black uppercase tracking-[0.2em] pt-1">
                      <p>Дата регистрации: {profileData.memberSince} • Клан: {profileData.clan}</p>
                    </div>
                  </div>

                  <div className="bg-white/5 p-4 rounded-[24px] border border-white/5 flex gap-3 h-fit">
                      <div className="achievement-card w-10 h-10 bg-yellow-500/20 rounded-xl flex items-center justify-center text-xl">🏆</div>
                      <div className="achievement-card w-10 h-10 bg-orange-500/20 rounded-xl flex items-center justify-center text-xl">🔥</div>
                  </div>
                </div>

                <div className="flex mt-8 p-1 bg-black/40 rounded-xl">
          ...
          {view === 'feed' && (
             <div className="px-2 mb-4"><h1 className="text-3xl font-black tracking-tight uppercase italic sss-logo">Global Stream</h1></div>
          )}

          {(view === 'feed' || (isOwnProfile && activeTab === 'posts')) && (
            <div className="glass-card rounded-[28px] p-6 shadow-xl">
              <div className="flex gap-4">
                <div className="w-11 h-11 rounded-xl bg-white/5 flex items-center justify-center text-2xl">{profileData.avatar_emoji || '🤩'}</div>
                <textarea 
                  placeholder="Что нового?" 
                  value={postText}
                  onChange={(e) => setPostText(e.target.value)}
                  className="flex-1 bg-transparent border-none outline-none text-[16px] py-1 resize-none h-16 placeholder:text-gray-800 text-white font-medium"
                />
              </div>
              <div className="flex justify-end mt-3">
                <button onClick={handlePublish} className="bg-white text-black px-8 py-2.5 rounded-xl font-black text-[11px] uppercase hover:bg-gray-200 active:scale-95 transition-all">Опубликовать</button>
              </div>
            </div>
          )}
          ...
          <div className="flex flex-col gap-4 mb-10">
            {posts.length > 0 && posts.map((post) => (
              <div key={post.id} className="glass-card rounded-[28px] p-6 transition-all group hover:border-white/10">
                <div className="flex justify-between items-start mb-4">
                  <div className="flex items-center gap-3">
                    <div onClick={() => goToProfile(post.username)} className="w-10 h-10 rounded-xl bg-gradient-to-br from-gray-800 to-[#111] flex items-center justify-center text-xl cursor-pointer hover:scale-105 transition-transform">
                      {post.avatar_emoji || '🤩'}
                    </div>
                    <div>
                      <p onClick={() => goToProfile(post.username)} className="font-black text-[13px] uppercase tracking-tight text-white/90 cursor-pointer hover:text-[#ff2a5f] transition-colors">{post.username}</p>
                  ...
                  {(post.username === user.username || (isOwnProfile && activeTab === 'reposts')) && (
                    <button onClick={() => handleDelete(post.id)} className="opacity-0 group-hover:opacity-100 p-2 text-gray-700 hover:text-red-500 transition-all cursor-pointer">
                      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="2.5"><path d="M6 18L18 6M6 6l12 12" /></svg>
                    </button>
                  )}
                </div>
                <p className="pl-1 text-[15px] text-gray-300 leading-normal mb-6">{post.content}</p>
                ...
                {activePostComments[post.id] && (
                  <div className="mt-4 pt-4 border-t border-white/5 space-y-4">
                    ...
                    <div className="flex gap-2">
                      ...
                      />
                      <button onClick={() => handleSendComment(post.id)} className="p-2 bg-[#ff2a5f] rounded-xl flex items-center justify-center">
