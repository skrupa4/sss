...
const ProfilePage = ({ user, onLogout, onUpdateUser }) => {
  ...
  // СОСТОЯНИЕ ДЛЯ КАСТОМНОГО МОДАЛЬНОГО ОКНА ПОДТВЕРЖДЕНИЯ
  const [confirmModal, setConfirmModal] = useState({
    isOpen: false,
    type: null, // 'post' | 'comment'
    targetId: null,
    extraId: null // Используется для postId при удалении комментария
  });
  ...
  // Инициация кастомного окна удаления поста
  ...
  // Инициация кастомного окна удаления комментария
  ...
  // Подтвержденное удаление из кастомного модального окна
  const executeConfirmDelete = async () => {
    const { type, targetId, extraId } = confirmModal;
    setConfirmModal({ isOpen: false, type: null, targetId: null, extraId: null });

    if (type === 'post') {
      try {
        const isRepostDelete = activeTab === 'reposts' && isOwnProfile;
        const url = isRepostDelete
          ? `${API_BASE_URL}/api/posts/${targetId}/repost`
          : `${API_BASE_URL}/api/posts/${targetId}`;
      ...
      try {
        const response = await fetch(`${API_BASE_URL}/api/posts/${extraId}/comments/${targetId}`, {
          method: 'DELETE',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ username: user.username })
        });
        ...
        if (response.ok) {
          setActivePostComments(prev => ({
            ...prev,
            [extraId]: (prev[extraId] || []).filter(c => c.id !== targetId)
          }));
          setPosts(prev => prev.map(p => p.id === extraId ? { ...p, comments_count: Math.max(0, (p.comments_count || 0) - 1) } : p));
        }
  ...
  return (
    <div className="min-h-screen bg-[#080808] text-white flex justify-center items-start pt-4 md:pt-8 px-3 md:px-6 pb-24 lg:pb-8 antialiased"
      ...
      {/* КРАСИВОЕ ВСКИДЫВАЕМОЕ ОКНО ПОДТВЕРЖДЕНИЯ В СТИЛЕ SSS */}
      {confirmModal.isOpen && (
        <div className="fixed inset-0 z-[10000] flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-black/60 backdrop-blur-md transition-opacity"></div>
          <div className="glass-card w-full max-w-[360px] rounded-[24px] p-6 border border-white/10 bg-[#0c0c0c]/90 relative z-10 shadow-2xl shadow-[#ff2a5f]/5 text-center animate-content visible">
            <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-[#ff2a5f] to-[#7e22ce] flex items-center justify-center mx-auto mb-4 border border-white/10 shadow-lg shadow-[#ff2a5f]/20">
              <svg className="w-5 h-5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="2.5">
                <path strokeLinecap="round" strokeLinejoin="round" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-4v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
              </svg>
            </div>
            <h3 className="text-[16px] font-black uppercase tracking-wide text-white">Вы уверены?</h3>
            <p className="text-[12px] font-medium text-gray-400 mt-2 leading-relaxed">
              Это действие полностью удалит данные из базы данных SSS без возможности восстановления.
            </p>
            <div className="flex gap-3 mt-5">
              <button
                onClick={() => setConfirmModal({ isOpen: false, type: null, targetId: null, extraId: null })}
                className="flex-1 h-10 rounded-xl border border-white/10 bg-white/5 hover:bg-white/10 text-white font-black text-[10px] uppercase tracking-wider transition-all"
              >
                Отмена
              </button>
              <button
                onClick={executeConfirmDelete}
                className="flex-1 h-10 rounded-xl bg-gradient-to-r from-[#ff2a5f] to-[#7e22ce] hover:scale-[1.02] active:scale-[0.98] text-white font-black text-[10px] uppercase tracking-wider transition-all shadow-lg shadow-[#ff2a5f]/10"
              >
                Удалить
              </button>
            </div>
                            ...
                            {(activePostComments[post.id] || []).map((comment) => (
                              <div key={comment.id} className="bg-white/[0.02] border border-white/5 p-3 rounded-2xl flex flex-col relative group/comment">
                                <div className="flex justify-between items-start mb-1">
                                  ...
                                  {/* ВЕРНУЛ КРЕСТИК УДАЛЕНИЯ КОММЕНТАРИЯ (СВЯЗАН С БД) */}
                                  {(comment.username === user.username || post.username === user.username) && (
                                    <button
                                      onClick={() => openDeleteCommentModal(post.id, comment.id)}
                                      className="text-gray-500 hover:text-red-500 font-bold text-[11px] p-1 transition-all"
