...
const ProfilePage = ({ user, onLogout, onUpdateUser }) => {
  ...
  useEffect(() => {
    if (view === 'messages' && activeChat) scrollToBottom();
  ...
  const handleSendMessage = async () => {
    ...
    try {
      const res = await fetch(`${API_BASE_URL}/api/messages`, {
        ...
        body: JSON.stringify({ sender: user.username, receiver: activeChat.username, content: textToSend })
      });
      if (res.ok) {
  ...
  const loadNotifications = useCallback(async () => {
    ...
    try {
      const res = await fetch(`${API_BASE_URL}/api/notifications?username=${user.username}`);
      if (res.ok) {
        const data = await res.json();
  ...
  const getNotificationText = (notif) => {
    switch (notif.type) {
      case 'follow': return `@${notif.user_from} подписался на вас`;
      case 'like': return `@${notif.user_from} лайкнул ваш пост`;
      case 'comment': return `@${notif.user_from} прокомментировал: "${notif.content?.substring(0, 50)}${notif.content?.length > 50 ? '…' : ''}"`;
      case 'repost': return `@${notif.user_from} репостнул ваш пост`;
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      if (view === 'messages') {
        await loadChats();
        if (activeChat) await loadMessages(activeChat.username);
  ...
  const handleDelete = async (postId) => {
    try {
      ...
      if (response.ok) setPosts(prev => prev.filter(p => p.id !== postId));
    } catch (err) { console.error(err); }
  };
  ...
  const addEmojiToComment = (postId, emoji) => {
    ...
    const before = text.substring(0, start);
    const after = text.substring(end, text.length);
    setCommentInputs(prev => ({ ...prev, [postId]: before + emoji + after }));
    setTimeout(() => {
      input.focus();
  ...
  const ActionButtons = ({ isMobile = false }) => {
    ...
    return (
      <>
        {isOwnProfile ? (
          <button onClick={handleSaveProfile} className={`${btnClasses} ${isEditing ? 'bg-[#ff2a5f] border-[#ff2a5f] text-white shadow-lg shadow-[#ff2a5f]/20' : 'bg-white/5 border-white/10 text-white hover:bg-white hover:text-black'}`}>
            {isEditing ? (
              <svg className={isMobile ? "w-3 h-3" : "w-3.5 h-3.5"} fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="3"><path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" /></svg>
            ) : (
              <svg className={isMobile ? "w-3 h-3" : "w-3.5 h-3.5"} fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="2.5"><path strokeLinecap="round" strokeLinejoin="round" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>
            )}
            <span>{isEditing ? 'Сохранить' : 'Редактировать'}</span>
          </button>
        ) : (
          <>
            <button onClick={handleFollow} className={`btn-fixed flex-1 ${isMobile ? 'px-3 py-1.5 h-8 rounded-lg text-[9px]' : 'md:flex-none px-5 py-2.5 h-11 rounded-xl text-[10px]'} font-black uppercase transition-all cursor-pointer border ${profileData.isSubscribed ? 'bg-white/5 border-white/10 text-white' : 'bg-[#ff2a5f] border-[#ff2a5f] text-white shadow-lg shadow-[#ff2a5f]/20 hover:scale-105 active:scale-95'}`}>
              {profileData.isSubscribed ? 'Отписаться' : 'Подписаться'}
            </button>
            <button onClick={() => { setActiveChat({ username: profileData.username }); loadMessages(profileData.username); setView('messages'); }} className={`btn-fixed flex-1 bg-white/5 border-white/10 text-white hover:bg-white hover:text-black ${isMobile ? 'px-3 py-1.5 h-8 rounded-lg text-[9px]' : 'md:flex-none px-5 py-2.5 h-11 rounded-xl text-[10px]'} font-black uppercase transition-all flex items-center justify-center gap-1.5`}>
              <svg className={isMobile ? "w-3 h-3" : "w-3.5 h-3.5"} fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="2.5"><path strokeLinecap="round" strokeLinejoin="round" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" /></svg>
              <span>ЛС</span>
            </button>
          </>
        )}
      ...
      <div className="min-h-screen bg-[#080808] text-white flex justify-center items-start pt-4 md:pt-8 px-3 md:px-6 pb-24 lg:pb-8 antialiased" style={{ fontFamily: 'Inter, system-ui, sans-serif' }}>
        ...
        <div className="w-full max-w-[1100px] flex flex-col lg:flex-row gap-6 justify-center">
          <aside className="fixed bottom-0 left-0 w-full lg:w-[240px] lg:static flex flex-row lg:flex-col justify-between lg:justify-start gap-5 glass-card p-3 sm:p-4 lg:p-5 rounded-t-[24px] lg:rounded-[28px] h-fit lg:sticky lg:top-8 shadow-2xl z-[999] border-t border-white/10 lg:border-none">
            ...
            <nav className="flex flex-row lg:flex-col gap-1 w-full lg:w-auto justify-around lg:justify-start">
              ...
              <div onClick={() => handleViewChange('notifications')} className={`flex items-center justify-center lg:justify-start gap-3 p-3 cursor-pointer rounded-xl transition-all duration-300 flex-1 lg:flex-none bg-white/5 text-white lg:border-r-2 lg:border-[#ff2a5f]`}>
                ...
                {unreadCount > 0 && <span className="ml-auto bg-red-500 text-white text-[10px] px-1.5 py-0.5 rounded-full">{unreadCount}</span>}
              </div>
              <div onClick={() => handleViewChange('profile', user.username)} className={`flex items-center justify-center lg:justify-start gap-3 p-3 cursor-pointer rounded-xl transition-all duration-300 flex-1 lg:flex-none text-gray-500 hover:text-white hover:bg-white/5`}>
          ...
          <main className="flex-1 w-full max-w-[680px]">
            <div className="glass-card rounded-2xl md:rounded-[32px] overflow-hidden shadow-2xl border-white/10">
              <div className="p-6 border-b border-white/5 flex justify-between items-center">
                <h1 className="text-xl md:text-2xl font-black tracking-tight uppercase italic sss-logo">Уведомления</h1>
                {unreadCount > 0 && (
                  <button onClick={markAllAsRead} className="text-[10px] font-black uppercase bg-white/10 px-3 py-1.5 rounded-full hover:bg-white/20">
                    Все прочитано
                  </button>
                )}
              </div>
              <div className="divide-y divide-white/5">
                {notificationsList.length === 0 ? (
                  <div className="p-8 text-center text-gray-500 font-bold text-xs uppercase">Нет уведомлений</div>
                ) : (
                  notificationsList.map(notif => (
                    <div key={notif.id} className={`p-4 transition-all ${!notif.is_read ? 'bg-white/5' : 'opacity-70'}`}>
                      <div className="flex gap-3">
                        <div className={`w-10 h-10 rounded-xl bg-gradient-to-br ${getAvatarGradient(notif.user_from)} flex items-center justify-center text-white font-black text-lg uppercase`}>
                          {notif.user_from.charAt(0)}
                        </div>
                        <div className="flex-1">
                          <p className="text-sm font-medium text-white/90">{getNotificationText(notif)}</p>
                          <p className="text-[10px] text-gray-500 mt-1">{new Date(notif.created_at).toLocaleString()}</p>
                        </div>
                        {!notif.is_read && <div className="w-2 h-2 bg-[#ff2a5f] rounded-full mt-2"></div>}
                      </div>
                    </div>
                  ))
                )}
  ...
  // ========== ОСНОВНОЙ РЕНДЕР (профиль, лента, сообщения) ==========
  return (
    <div className="min-h-screen bg-[#080808] text-white flex justify-center items-start pt-4 md:pt-8 px-3 md:px-6 pb-24 lg:pb-8 antialiased" style={{ fontFamily: 'Inter, system-ui, sans-serif' }}>
      <style>{`
        @keyframes flow { 0% { background-position: 0% 50%; } 50% { background-position: 100% 50%; } 100% { background-position: 0% 50%; } }
              ...
              <div className={`w-full md:w-2/5 border-r border-white/5 flex flex-col ${activeChat ? 'hidden md:flex' : 'flex'}`}>
                ...
                <div className="flex-1 overflow-y-auto p-2 space-y-1 chat-scroll">
                  {chats.length === 0 ? (
                    ...
                    chats.map(chat => (
                      <div key={chat.username} onClick={() => { setActiveChat(chat); loadMessages(chat.username); }} className={`flex items-center gap-3 p-3 rounded-xl cursor-pointer transition-all ${activeChat?.username === chat.username ? 'bg-white/5 border border-white/5' : 'hover:bg-white/[0.02] border border-transparent'}`}>
                ...
                {activeChat ? (
                  <>
                    ...
                    <div className="p-3 border-t border-white/5 bg-white/[0.01] flex gap-2 items-center flex-shrink-0">
                      <input type="text" placeholder="Написать сообщение..." className="comment-input h-10 md:text-sm" value={messageInput} onChange={(e) => setMessageInput(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleSendMessage()} />
                  ...
                  <div className="flex flex-col items-center gap-2 select-none opacity-40">
                    <svg className="w-8 h-8 text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="2"><path strokeLinecap="round" strokeLinejoin="round" d="M7 8h10M7 12h4m1 8l-4-4H5a2 2 0 01-2-2V5a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-3l-4 4z" /></svg>
          ...
          {view !== 'messages' && view !== 'notifications' && (view === 'feed' || (isOwnProfile && activeTab === 'posts')) && (
            <div className="glass-card rounded-2xl md:rounded-[28px] p-4 md:p-6 shadow-xl relative">
              <div className="flex gap-3 md:gap-4">
                ...
                <textarea ref={postInputRef} placeholder="Что нового?" value={postText} onChange={(e) => setPostText(e.target.value)} className="flex-1 bg-transparent border-none outline-none text-sm md:text-[16px] py-1 resize-none h-16 placeholder:text-gray-800 text-white font-medium" />
              </div>
              {showPostEmoji && (
                <div className="emoji-bar mt-3 p-2.5 rounded-xl flex flex-wrap gap-2 max-h-24 overflow-y-auto chat-scroll animate-fadeIn">
                  {EMOJI_LIST.map((emoji, i) => <button key={i} onClick={() => addEmojiToPost(emoji)} className="emoji-item text-lg md:text-xl p-0.5 outline-none select-none">{emoji}</button>)}
                </div>
              )}
              <div className="flex justify-between items-center mt-3">
                <button onClick={() => setShowPostEmoji(!showPostEmoji)} className={`p-2 rounded-xl transition-all border ${showPostEmoji ? 'bg-[#ff2a5f]/10 border-[#ff2a5f]/30 text-[#ff2a5f]' : 'bg-white/5 border-white/5 text-gray-400 hover:text-white hover:bg-white/10'}`}>
                  <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="2.5"><path strokeLinecap="round" strokeLinejoin="round" d="M14.828 14.828a4 4 0 01-5.656 0M9 10h.01M15 10h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                </button>
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <div className="flex items-center gap-1.5">
                            ...
                            {post.is_verified && <div className="w-3.5 h-3.5 bg-[#ff2a5f] rounded-full flex items-center justify-center text-[7px] text-white font-bold shadow-[0_0_8px_rgba(255,42,95,0.4)] flex-shrink-0" title="Верифицированный аккаунт">✓</div>}
                          </div>
                      ...
                      <div className="mt-4 pt-4 border-t border-white/5 space-y-4">
                        ...
                        {showCommentEmoji[post.id] && (
                          <div className="emoji-bar p-2 rounded-xl flex flex-wrap gap-1.5 max-h-20 overflow-y-auto chat-scroll animate-fadeIn">
                            {EMOJI_LIST.map((emoji, i) => <button key={i} onClick={() => addEmojiToComment(post.id, emoji)} className="emoji-item text-base p-0.5 outline-none select-none">{emoji}</button>)}
                          </div>
                        )}
                        <div className="flex gap-2 items-center">
                          <button onClick={() => setShowCommentEmoji(prev => ({ ...prev, [post.id]: !prev[post.id] }))} className={`p-2.5 h-10 rounded-xl transition-all border ${showCommentEmoji[post.id] ? 'bg-[#ff2a5f]/10 border-[#ff2a5f]/30 text-[#ff2a5f]' : 'bg-white/5 border-white/5 text-gray-400 hover:text-white hover:bg-white/10'}`}>
                            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="2.5"><path strokeLinecap="round" strokeLinejoin="round" d="M14.828 14.828a4 4 0 01-5.656 0M9 10h.01M15 10h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                          </button>
                          <input ref={el => commentInputRefs.current[post.id] = el} type="text" placeholder="Написать... " className="comment-input h-10" value={commentInputs[post.id] || ''} onChange={(e) => setCommentInputs(prev => ({ ...prev, [post.id]: e.target.value }))} onKeyDown={(e) => e.key === 'Enter' && handleSendComment(post.id)} />
                ...
                view === 'profile' && (
                  <div className="glass-card rounded-2xl md:rounded-[28px] p-8 text-center border-dashed border-white/5 select-none">
                    <p className="text-gray-600 font-black uppercase text-[11px] tracking-[0.2em]">
                      {activeTab === 'posts' ? (isOwnProfile ? 'Вы еще не опубликовали ни одной записи' : 'Пользователь еще не опубликовал записи') : (isOwnProfile ? 'Вы еще не сделали ни одного репоста' : 'Пользователь еще не сделал ни одного репоста')}
                    </p>
                  </div>
                )
              )}
